...
Bubbles = () ->
  ...
  # this scale will be used to size the bubbles
  ...
  tick = (e) ->
    ...
    node
      ...
      .each(collide(jitter))
      .attr("transform", (d) -> "translate(#{d.x},#{d.y})")  #to move to new location


    # As the labels are created in raw html and not svg, make sure "px" is specified for proper movement
  ...
  # The force variable is the force layout controlling the bubbles, we are disabling gravity and charge as we use our own implementation 
  ...
  chart = (selection) ->
    selection.each (rawData) ->
      ...
      # first, transform the data in the right format
  ...
  updateLabels = () ->
    # as in updateNodes, we use idValue to define what the unique id for each data point is
    ...
    label
      ...
      #CASE 2 check: bubble width is large for word, if yes then find the appropriate position for the label
    #------------------------------------------------
  ...
  gravity = (alpha) ->
    ...
    # use alpha to affect how much to push towards the horizontal or vertical (in this case, we push more vertically since we want
    # the nodes to align themselves horizontally)
    ax = alpha / 8
    ay = alpha
...
$ ->
  ...
  # load our data
  d3.json("data/sherlock_word_count.json", display)